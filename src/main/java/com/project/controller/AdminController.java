...
public class AdminController { //public 접근제어 admincontroller class에 대한 접근을 제어함
	...
	public String list(Model model, @ModelAttribute("scri") SearchCriteria scri) throws Exception{  //list메소드를 선언해주고  파리미터 model을 넣어주고 보낸다.
		logger.info("list"); //list의 로그찍기
		
		service.list(model, scri); //model에 값을 넣을때에는 addAttribute를 사용한다.
